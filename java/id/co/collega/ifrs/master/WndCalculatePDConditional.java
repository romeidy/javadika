...
@Scope("desktop")
public class WndCalculatePDConditional extends SelectorComposer<Component> {

	private static final Logger LOGGER = LoggerFactory.getLogger(WndCalculatePDConditional.class);
		...
		btnProses.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			public void onEvent(Event event) throws Exception {
				if (validation()) {
					...
					Aksi = "Start Calculated PD Unconditional "+ComponentUtil.getValue(txtPeriode);
					doLogAktfitas(Aksi);
