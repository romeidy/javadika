...
@Scope("desktop")
public class WndKreditModifikasi extends SelectorComposer<Component>{
...
//	@Wire Intbox 	txtTenggangBunga;
	...
	private List listAngsuranUpload =new ArrayList();
	private List listErr=new ArrayList();
	...
	public void doAfterCompose(Component comp) throws Exception  {
		...
		btnGenerateJadwal.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event e) throws Exception {
		...
		rdJadwalAngsur.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event e) throws Exception {
				...
				if (isjadwalAngsur.equals(1)) {
					btnGenerateJadwal.setDisabled(false);
					doGenerateArusKasBaru();
					btnUploadJadwal.setDisabled(true);
				} else {
					if (listAngsuranUpload.size() == 0) {
						btnGenerateJadwal.setDisabled(true);
		...
		txtTglMulaiBaru.addEventListener("onBlur", new EventListener() {
			public void onEvent(Event e) throws Exception {
				...
				Integer tenor = (Integer) ComponentUtil.getValue(txtTenorBaru);
				if (tglMulai != null && tenor != null) {
				    if(tenor > 999)
				        throw new WrongValueException(txtTenorBaru, "Nilai maksimal 999");
	...
	protected void resetArusKasBaru() {
		...
		decModifikasi.setValue(BigDecimal.ZERO);
	}
	...
	private void doReadFile(InputStream fileName) {
		...
		listAngsuranUpload = new ArrayList<>();
		 ...
		 Integer intJangkaWktNew = (Integer) ComponentUtil.getValue(txtTenorBaru);
...
//	                	 MessageBox.showInformation("bln_ke "+bln_ke +" intPeriode "+intPeriode);
	...
	public void doGenerateArusKasUpload(List angsurUpload) {
		Date tglMulai = (Date) ComponentUtil.getValue(txtTglMulaiBaru);
		int jangkaWaktu = (Integer) ComponentUtil.getValue(txtTenorBaru);
...
//				dtoMap.put("NILAI_KINI_ARUS", BigDecimal.ZERO);
		...
		if(selisih > 0){
			decModifikasi.setValue( new BigDecimal(selisih));
		}else if(selisih < 0){
			decModifikasi.setValue( new BigDecimal(selisih*-1));
		}else{
			decModifikasi.setValue( new BigDecimal(0.00));
		}
	...
	public boolean validation() {
		...
		if (isValid) {
			Integer jangkaWaktu=(Integer) ComponentUtil.getValue(txtTenorBaru);
			if (jangkaWaktu == null) {
				isValid = false;
				wrongValue.add(new WrongValueException(txtTenorBaru, "Jangka Waktu Harus Diisi"));
			}
	...
	private void doLoadDataRek(String noRek) {
		...
		if (dataRek!=null) {
			if (dataRek.getInt("ACCSTS").equals(0)) {
				...
				ComponentUtil.setValue(txtDPD, dataRek.getInt("DPD"));
...
//		wnd.getChildren().clear();
		...
		ComponentUtil.setValue(rdJadwalAngsur, 1);
		btnGenerateJadwal.setDisabled(false);
		btnUploadJadwal.setDisabled(true);
