...
public class MasterServicesImpl implements MasterServices{
	...
	@SuppressWarnings("unchecked")
	public List<DTOMap> getPrivItemsByUser(String user){
		String sql = "	SELECT B.USERID, B.MENUID, A.NAME, C.FLGINSERT, C.FLGUPDATE, C.FLGDELETE "
				+ "		FROM CFG_MENU A																"
				+ "						LEFT OUTER JOIN CFG_USER_ROLE_MENU B ON A.MENU_ID = B.MENUID "
				+ "						LEFT OUTER JOIN CFG_USER_PRV_MENU C ON A.MENU_ID = C.MENUID AND B.USERID = ?"
