...
public class WndPenempatanBankLain extends SelectorComposer<Component> {
	...
	String userid = mapUser.getString("USERID");	String menuid = (String) GlobalVariable.getInstance().get("menuid");
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		btnDelete.addEventListener(Events.ON_CLICK, new EventListener() {
			public void onEvent(Event event) throws Exception {
				if(checkPrivDelete(userid, menuid)){
					System.out.println(menuid);
					doDelete();					
				} else {
					MessageBox.showInformation("Maaf, Anda tidak berhak.");				
				}
			}
		});
...
//				Validasi Bunga jika lebih dari 100 maka error, jika kosong maka set 0.0000
	...
	private void doSave() {
		if (doValidation()) {
			if (onLoad) {
				if(checkPrivUpdate(userid, menuid)){
					doUpdate();					
				} else {
					MessageBox.showInformation("Maaf, Anda tidak berhak.");				
				}
			} else {
				if(checkPrivInsert(userid, menuid)){
					doInsert();					
				} else {
					MessageBox.showInformation("Maaf, Anda tidak berhak.");				
				}
